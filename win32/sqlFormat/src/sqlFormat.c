...
static char* sendPost(char* url, char* formatSql, char* formatOptions){
	...
	CStringAppend(paramBuffer, "rqst_db_vendor=oracle&rqst_isf_client=c_client&rqst_input_sql=");
...
int main(int argc,char *argv[])
{
	int argIndex, index;
	List *argList;
	FILE *sqlFile, *formatOptionsFile, *formatResult;
	char *sqlText = "SELECT 1+1 FROM ABC WHERE  output = \"Hello world!\" ";
	CString *formatSql, *formatOptions = NULL;
	char* response;
	...
	if(argIndex!=-1 && argList->size(argList) >argIndex+1){
		formatResult=fopen(argv[argIndex+1],"w");
	}
	else formatResult = NULL;

	argIndex = argList->indexOf(argList, "-i");
	if(argIndex!=-1 && argList->size(argList) >argIndex+1){
		...
		sqlFile = NULL;
		formatSql = CStringNew();
		CStringAppend(formatSql, sqlText);
	}
	...
	if(json!=NULL){
		...
		if(formatItem!=NULL){
			printResult(formatResult, "%s\n", formatItem->valuestring);
			free(formatItem);
		}
		else{
			fprintf(stderr,"Failed to format sql\n");
			fprintf(stderr,"Return json: %s\n", response );
		}
		free(json);
	}
	else{
